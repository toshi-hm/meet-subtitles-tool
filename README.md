...
- Meetごとの字幕データは、IndexedDBへの書き込み開始から24時間後に自動削除されます。
- 自動削除はMeetページを開いた時、またはMeetページを開いたまま定期確認が行われた時に実行されます。24時間経過後にMeetを開いていない場合は、次回Meetページを開いた時に削除されます。
...
Meetの字幕欄に表示名が提供されない場合は、メールアドレスを保存せず `自分` と表示します。Meet画面上で表示名を取得できる場合は、その表示名を使用します。
