...
会議に入室する前の待機画面でも、Meet Subtitlesパネルの `Drive接続` を押してOAuthを先に完了できます。字幕本文は送信されず、認証だけが行われます。認証後はボタンが `Drive保存` に変わります。
...
1. Google Meetの待機画面を開きます。
2. 入室前にDrive認証を済ませる場合は、パネルの `Drive接続` を押し、Googleの認証画面で許可します。
3. 会議に入室すると、画面内に `Meet Subtitles` パネルが表示され、字幕が自動でONになります。
4. 会議中は字幕が自動的に蓄積されます。パネルには保存件数が表示されます。
5. 必要な時点で `コピー` または `TXT保存` を押します。
6. パネルを小さくしたいときは、右上の `−` を押します。ヘッダーをドラッグすると位置を移動できます。
